...
9. 固定短语
    ...
    * one the way(在路上)
10. 乐器名称前面
    * He began to play `the` violir at the age of 5.

---

### 不定冠词

泛指：表示一类事物，或者某种事物的总称(the an a)

1. 当不可数名词不加冠词表泛指
    * `Life` is hard sometimes.
    * I am writing a book about `the life` of my hard time.
2. 复数不加冠词表示泛指
    * `Houses` become more and more expensive.
    * I prefer `the house` near by the lake
...
a mile and two == one or two miles

1. 强调'一'这个数量/讲故事 - 只能用 one
    * There is `one` person……
    * There is `one` place……
...
6. 节假日前不用
    * On `Children's Day` the boys often get present from their parents.
7. 球类名词前不用
